...
let convertSolar2Lunar = (dd, mm, yy, timeZone = TIMEZONE) => {
...
let convertLunar2Solar = (lunarDay, lunarMonth, lunarYear, timeZone = TIMEZONE, lunarLeap = 0) => {
...
let getCanChi = (y, m, d, t) => {
    ...
    return {
        ...
        "canchi": {"y": yy, "m": mm, "d": dd, "t": hh},
    }
}

let getLunar = (y, m, d, t) => {
    return getLunarDate(d, m, y); // 그냥 jd를 구하는 함수
}

let solar2Lunar = (y, m, d) => {
    var [dd, mm, yyyy, leap] = convertSolar2Lunar(d, m, y, TIMEZONE);
    return {"y": yyyy, "m": mm, "d": dd, "l": leap}
}

let lunar2Solar = (y, m, d) => {
    var [dd, mm, yyyy] = convertLunar2Solar(d, m, y, TIMEZONE);
    return {"y": yyyy, "m": mm, "d": dd}
}

let isValidLunar = (y, m, d) => {
    var tmp = {"y": y, "m": m, "d": d}
    var solar = lunar2Solar(y, m, d);
    var lunar = solar2Lunar(solar.y, solar.m, solar.d);

    // console.log(`lunar:${JSON.stringify(tmp)}`)
    // console.log(`solar:${JSON.stringify(solar)}`)
    // console.log(`lunar2:${JSON.stringify(lunar)}`)

    return y == lunar.y && m == lunar.m && d == lunar.d
}
